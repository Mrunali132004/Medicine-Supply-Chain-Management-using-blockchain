...
export default function CreateMedicine(props) {
  ...
  const handleSubmit = (e) => {
    ...
    // Validate inputs
    if (!isValidAddress(rawMatAddress)) {
      alert('You have entered an incorrect raw material address. Please check and try again.');
      isLoading(false);
      return; // Prevent further processing
    }

    if (!isValidAddress(manufacturerAddress)) {
      alert('You have entered an incorrect manufacturer address. Please check and try again.');
      isLoading(false);
      return; // Prevent further processing
    }
