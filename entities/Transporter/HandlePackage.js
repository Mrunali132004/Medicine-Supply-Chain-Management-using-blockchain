...
export default function HandlePackage(props) {
  ...
  const handleSubmit = (e) => {
    ...
    // You can add additional validation for 'type' and 'cid' if needed

    supplyChain.methods.transporterHandlePackage(pAddress, type, cid).send({ from: account })
      .once('receipt', async (receipt) => {
        console.log(receipt);
        alert('Package handled successfully!');
        isLoading(false);
      })
      .catch((error) => {
        alert('An error occurred while handling the package. Please try again.');
        console.error(error);
        isLoading(false);
      });
  };
